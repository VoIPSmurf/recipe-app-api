...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_full_update(self):
        ...
        payload = {
            ...
            'description': 'New description of a recipe.',
            'time_minutes': 10,
            'price': Decimal('99.99'),
        }
    ...
    def test_create_recipe_with_existing_tags(self):
        ...
        tag_example = Tag.objects.create(user=self.user, name='Example')
        payload = {
            'title': 'Sample recipe',
            'time_minutes': 30,
            'price': Decimal('5.99'),
        ...
        recipe = recipes[0]
        self.assertEqual(recipe.tags.count(), 2)
        self.assertIn(tag_example, recipe.tags.all())
        for tag in payload['tags']:
            exist = recipe.tags.filter(
                name=tag['name'],
                user=self.user,
    ...
    def test_create_recipe_with_new_ingredients(self):
        ...
        for ingredient in payload['ingredients']:
            exists = (recipe.ingredients
                            .filter(user=self.user, name=ingredient['name'])
                            .exists())
            self.assertTrue(exists)

    def test_create_recipe_with_existing_ingredient(self):
        """Test creating rexipe with existing ingredient."""
        ingredient = (Ingredient.objects.
                      create(user=self.user, name='Ingredient 1'))
        payload = {
            'title': 'Sample recipe',
            'time_minutes': 30,
            'price': Decimal('5.99'),
    ...
    def test_create_ingredient_on_update(self):
        ...
        res = self.client.patch(url, payload, format='json')
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_ingredient = (Ingredient.objects
                                    .get(user=self.user,
                                         name='New ingredient',
                                         ))
    ...
    def test_update_recipe_assign_ingredient(self):
        """Test update recipe and assign an ingredient."""
        ingredient1 = (Ingredient.objects.
                       create(user=self.user, name="Ingredient 1"))
        recipe = create_recipe(user=self.user)
        recipe.ingredients.add(ingredient1)

        ingredient2 = (Ingredient.objects.
                       create(user=self.user, name="Ingredient 2"))
    ...
    def test_clear_recipe_ingredients(self):
        ...
        ingredient = (Ingredient.objects.
                      create(user=self.user, name="Ingredient"))
        recipe = create_recipe(user=self.user)
